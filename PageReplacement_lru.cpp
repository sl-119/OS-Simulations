//LEKHESH 22CSB0C03
//PAGE REPLACEMENT ALGO WITH LRU
...
struct PageTable {
    ...
    int page_faults;
    int last_used[MAX_FRAMES]; // To store last used time for LRU
};
...
// Function to find the position to replace using LRU
int find_pos_lru(PageTable *table, int np, int curr) {
    int min_time = 1000000, pos = -1;
    for (int i = 0; i < MAX_FRAMES; i++) {
        if (table->frames[i] == -1) return i; // Empty frame
        if (table->last_used[i] < min_time) {
            min_time = table->last_used[i];
            pos = i;
        }
    }
    return pos;
...
int main() {
    ...
    memset(table->last_used, 0, sizeof(table->last_used));
    table->page_faults = 0;
        ...
        if (fork() == 0) { // Child process
            for (int j = 0; j < np; j++) {
                ...
                if (!present(table, pages[j])) {
                    // Page fault, perform LRU replacement
                    int pos = find_pos_lru(table, np, j);
                    table->frames[pos] = pages[j];
                    table->last_used[pos] = j; // Update last used time
                    table->page_faults++;
                    print_table(table);
                } else {
                    // Update last used time if page is present
                    for (int k = 0; k < MAX_FRAMES; k++) {
                        if (table->frames[k] == pages[j]) {
                            table->last_used[k] = j;
                            break;
                        }
                    }
...
int find_pos_lru(int table_frame[],int nf,int pages[],int np,int curr)
{
	for(int i=0;i<nf;i++)
	{
		if(table_frame[i]==-1)
...
int main()
{
	...
	for(int i=0;i<np;i++)
	{
		...
		if(!present(table_frame,nf,pages[i]))
		{
			...
			//for lru
			int curr=i;
			pos=find_pos_lru(table_frame,nf,pages,np,curr);
